...
export const extractRouteFieldErrors = (error) => {
  const data = error?.response?.data;
...
export const routeService = {
  getAll: async ({ tuKhoa, page } = {}) => {
    const response = await axiosClient.get("/LoTrinh", {
      params: {
  ...
  getById: async (id) => {
    const response = await axiosClient.get(`/LoTrinh/${id}`);
    return response.data;
  },

  getAirportOptions: async () => {
    const response = await axiosClient.get("/LoTrinh/san-bay-options");
    return Array.isArray(response.data) ? response.data : [];
  },

  create: async (payload) => {
    try {
      const response = await axiosClient.post("/LoTrinh", payload);
      return response.data;
    } catch (error) {
  ...
  update: async (id, payload) => {
    try {
      const response = await axiosClient.put(`/LoTrinh/${id}`, payload);
      return response.data;
    } catch (error) {
      error.userMessage = extractErrorMessage(error);
      throw error;
  ...
  remove: async (id) => {
    try {
      const response = await axiosClient.delete(`/LoTrinh/${id}`);
      return response.data;
    } catch (error) {
      error.userMessage = extractErrorMessage(error);
      throw error;
